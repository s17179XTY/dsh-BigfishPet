      ...
      try {
        ...
        if (body && typeof body === 'object') {
          ...
          if (Array.isArray(body.bubbleStates)) {
            ...
            state.bubbleStates = body.bubbleStates.filter((v) => typeof v === 'string' && allowed.has(v))
          }
...
export function apply(ctx) {
  ...
  // 响应子 Agent 开关：从 pet.json 读取（默认 false），每 3s 同步一次。
  const syncReducer = async () => {
    try {
      const state = await readState()
      reducer.includeSubagents = state.includeSubagents === true
    } catch { /* keep current */ }
  }
  syncReducer()
  const reducerSyncTimer = setInterval(() => { syncReducer() }, 3000)
  if (reducerSyncTimer.unref) reducerSyncTimer.unref()
  ...
  ctx.effect(() => () => {
    if (statusWriteTimer) clearTimeout(statusWriteTimer)
    statusWriteTimer = null
    clearInterval(reducerSyncTimer)
    flushStatus()
  })
