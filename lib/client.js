...
window.__ModuleLoader__.load({ id: "bigfish-pet", factory: (require) => {
	...
	const ROW_STYLE = { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 20, flexWrap: "wrap" };
	const FIELD_TEXT_STYLE = { minWidth: 0, flex: "1 1 220px" };
	const CONTROL_STYLE = { flexShrink: 0 };
	// 照抄 dsh-dafeiyu：select/input 不硬编码 background/color/border，
	// 交给 DSH 主题变量处理（硬编码 transparent+inherit 在部分主题下会
	// 白底白字，导致选择框文字不可见）。
	const SELECT_STYLE = { minWidth: 120, padding: "6px 10px", borderRadius: 8 };
	const INPUT_STYLE = { minWidth: 140, padding: "6px 10px", borderRadius: 8 };
	...
	function PetSettings() {
		...
		// Capture the target up-front: React synthetic events must not be
		// touched inside the async setState/timeout callbacks.
		const debouncedWrite = (field, patchBuilder, delay = 250) => (event) => {
			const target = event.target;
			const value = target.value;
			const numeric = target.type === "number" || target.type === "range";
			patchSeq.current += 1;
			setState((prev) => {
				...
				cursor[keys[keys.length - 1]] = numeric ? Number(value) : value;
				return updated;
			});
			...
			const timer = setTimeout(() => {
				...
				void write(patchBuilder(numeric ? Number(value) : value));
			}, delay);
		...
		return h("li", { style: CARD_STYLE, "data-testid": "bigfish-pet-settings" },
			...
			h(Field, { label: "位置", hint: "重置为默认位置（右下角）。" },
				h("button", {
					disabled: busy,
					style: { padding: "6px 16px", borderRadius: 8, border: "1px solid var(--border-color, #d8d8d8)", background: "transparent", color: "inherit", cursor: "pointer" },
	...
	function statusBlock(state) {
		...
		const progress = status.progress && status.progress.total
			...
			: "—";
		return h("div", { style: { marginTop: "4px", padding: "12px 14px", borderRadius: 8, border: "1px solid var(--border-color, #d8d8d8)", fontSize: 13, lineHeight: 1.8, display: "grid", gridTemplateColumns: "auto auto", gap: "2px 24px", width: "max-content" } },
