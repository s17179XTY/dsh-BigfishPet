...
- **单一事实源**：`~/.dsh/pet.json`（name / display{visible,size,right,bottom} / notify / activity / status）。**无 affinity/treats**（亲密度系统已移除，POST /state 不再支持 action=pet|feed）。
...
- **活跃程度**：安静/标准/活泼 三选按钮（draft.activity，POST 时一并保存）。
